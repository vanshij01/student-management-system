    ...
    if (admissonForm) {
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-remove-document', function () {
                ...
                // Remove validation fields if they exist
                if (index) {
        ...
        function getCoursesByEducation(education_type) {
            $.ajax({
                type: "post",
                url: window.getCoursesByEducationUrl,
                data: {
        ...
        function getCourseList(course_id) {
            ...
            console.log("course_id", [course_id, student_id]);

            $.ajax({
                type: "get",
                url: "/admission/getStudentAdmissionDataBackend/" + course_id + "/" + student_id,
                cache: false,
                success: function (response) {
                    ...
                    var semesterhtml = "";
                    for (let i = 1; i <= totalSemesters; i++) {
                        semesterhtml += '<option value="' + i + '"' + ((current_semester > 0 && current_semester == i) ? ' selected' : '') + '>' + i + '</option>';
        ...
        var oldCourseEducationType = $('#education_type').val();
        ...
        function updateSemesterValidators(currentSemester) {
            ...
            var oldCourseEducationType = $('#education_type').val();
            const courseChanged = currentCourse !== courseId;
            ...
            if (oldCourseEducationType != 'HSC' && courseId != 27) {
                ...
                const fileValidators = {
                    validators: {
                        ...(optional ? {} : {
                            notEmpty: {
                                message: `Please upload Semester-${semNumber} result or backlog.`
                            }
                        }),
                        file: {
                            extension: 'jpg,jpeg,png',
        ...
        console.log('isJobOrInternship', isJobOrInternship);
            ...
            if (studentNew === 'true') {
                ...
                if (oldCourseEducationType == 'HSC') {
                    ...
                    [
                        ...
                        'ipcc_backlog_percentage', 'cpt_backlog_percentage', 'ca_final_backlog_percentage',
        ...
        displayBacklogResultFields(backlog, courseId);
        ...
        function displayBacklogResultFields(backlog, courseId) {
            if (backlog === 'true') {
                if (courseId == 27) {
                    $('.degree-results').addClass('d-none');
