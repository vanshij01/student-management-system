...
(function () {
    ...
    if (admissonForm) {
        ...
        function displayVehicleField(hasVehicle) {
            const vehicleFields = [
                ['is_have_helmet', {
                    notEmpty: { message: 'Please check the checkbox.' }
                }],
                ['vehicle_number', {
        ...
        const FormValidation2 = FormValidation.formValidation(admissonFormValStep2, {
            fields: {
                ...
                annual_income: {
                    ...
                    regexp: {
                        regexp: /^[1-9]\d*$/,
                        message: 'Annual income must be grater than 0.'
                    }
                },
                    ...
                    validators: {
                        ...
                        callback: {
                            ...
                            callback: function (input) {
                                ...
                                // const isUpdateMode = window.isUpdateMode || false;

                                if (window.isNewStudent === 'true' || isUpdateMode) {
                                    // Skip validation for new students
                                    return true;
                                }

                                // For new admissions, apply the original validation
        ...
        function getCourseList(course_id) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var current_semester = parseInt(last_semester) > 0 ? parseInt(last_semester) + 2 : 0;
        ...
        function updateSemesterValidators(currentSemester) {
            ...
            if (isBachelor) {
                if (currentSemester === 0) {
                    ...
                    // 2nd year Bachelor: Show Semester 1 and 2
                ...
                // 1st year Master: Show Semester 5 and 6
                if (currentSemester === 0) {
                    // 1st year Bachelor: No semester uploads needed
                    addSscHscLeavingValidators();
                } else if (currentSemester === 1 || currentSemester === 2) {
        ...
        //     const field = `semester_${semNumber}_result`;
        //     const semesterVal = $(`#semester${semNumber}Img`).attr('src');

        //     // Add validator for semester field
        //     FormValidation3.addField(field, semesterVal == null ? createFileValidator(`Please upload your Semester-${semNumber} result.`) : '');

        //     // Show the corresponding semester group
        ...
        const validationRules = {
            ...
            internship_letter: createFileValidator('Please upload your internship letter.'),
            job_letter: createFileValidator('Please upload your job letter.'),
        ...
        const educationDocMap = {
            HSC: (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate'] : ['ssc_result', 'leaving_certificate'],
            Diploma: (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate'] : ['ssc_result', 'hsc_result', 'leaving_certificate'],
            "Bachelor's Degree": (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate'] : ['ssc_result', 'hsc_result', 'leaving_certificate'],
            "Master's Degree": (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate', 'degree_certificate'] : ['ssc_result', 'hsc_result', 'leaving_certificate', 'last_qualification_result', 'degree_certificate'],
            "Master's Degree": (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate', 'degree_certificate'] : ['ssc_result', 'hsc_result', 'leaving_certificate', 'last_qualification_result', 'degree_certificate'],
            "Professional Degree": (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate', 'degree_certificate'] : ['ssc_result', 'hsc_result', 'leaving_certificate', 'last_qualification_result', 'degree_certificate'],
            Internship: (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate', 'degree_certificate', 'internship_letter'] : ['ssc_result', 'hsc_result', 'leaving_certificate', 'last_qualification_result', 'degree_certificate', 'internship_letter'],
            Job: (board_type) => board_type === 'Other' ? ['last_qualification_result', 'leaving_certificate', 'degree_certificate', 'job_letter'] : ['ssc_result', 'hsc_result', 'leaving_certificate', 'last_qualification_result', 'degree_certificate', 'job_letter'],
        };
        ...
        validateEducationDocuments(education_type, board_type);
        ...
        function validateEducationDocuments(education_type, board_type) {
            resetValidation();
