...
@section('content')
    <div class="card">
        <div class="card-header text-white py-3">
            <h5 class="card-title m-0 me-2 text-light">Update Room</h5>
        </div>
        <form action="{{ route('room.update', $room->id) }}" method="post" class="room_form">
            @csrf
            @method('PUT')
                ...
                <div class="row">
                    <div class="col-sm-6 col-lg-4 mb-4">
                        ...
                        <select name="hostel_id" id="hostel_id" class="select2 form-select" data-placeholder="Select hostel"
                            ...
                            @foreach ($hostels as $hostel)
                                <option value="{{ old('hostel_id', $hostel->id) }}"
                                    @if ($room->hostel_id == $hostel->id) selected @endif>
                    ...
                    <div class="col-sm-6 col-lg-4 mb-4">
                        ...
                        <input type="text" class="form-control" name="room_number"
                            value="{{ old('room_number', $room->room_number) }}" id="room_number"
