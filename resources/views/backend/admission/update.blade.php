...
@section('content')
    <section class="Personal_details_form">
        <div class="container">
            <a href="{{ route('admission.index') }}" class="go-back">
            ...
            <div id="admission_form" class="bs-stepper mt-2">
                <form action="{{ route('admission.update', $admissionDetail->id) }}" method="post"
                    enctype="multipart/form-data" id="admission_form">
                    {{-- {{ dd($studentData) }} --}}
                    @csrf
                    @method('PUT')
                    <input type="hidden" id="form_mode" name="form_mode" value="update">
                    <input type="hidden" value="{{ $studentData->id }}" name="student_id" id="student_id">
                    <input type="hidden" name="hdnStudentPhotourl" id="hdnStudentPhotourl"
                        value="{{ $admissionDetail->student_photo_url ?? '' }}">
                    <input type="hidden" name="hdnFatherPhotourl" id="hdnFatherPhotourl"
                        value="{{ $admissionDetail->father_photo_url ?? '' }}">
                    <input type="hidden" name="hdnMotherPhotourl" id="hdnMotherPhotourl"
                        value="{{ $admissionDetail->mother_photo_url ?? '' }}">
                    <input type="hidden" name="hdnLicencePhotourl" id="hdnLicencePhotourl"
                        value="{{ $admissionDetail->licence_doc_url ?? '' }}">
                    <input type="hidden" name="hdnInsurancePhotourl" id="hdnInsurancePhotourl"
                        value="{{ $admissionDetail->insurance_doc_url ?? '' }}">
                    <input type="hidden" name="hdnRcbookFrontDocPhotourl" id="hdnRcbookFrontDocPhotourl"
                        value="{{ $admissionDetail->rcbook_front_doc_url ?? '' }}">
                    <input type="hidden" name="hdnRcbookBackDocPhotourl" id="hdnRcbookBackDocPhotourl"
                        value="{{ $admissionDetail->rcbook_back_doc_url ?? '' }}">
                    ...
                    <div class="bs-stepper-content p-0">
                        <!-- personal Details -->
                        @include('backend.admission.update_partial._personal_details')
                        <!-- family-details -->
                        @include('backend.admission.update_partial._family_details')
                        <!-- education-details -->
                        @include('backend.admission.update_partial._education_details')
                        <!-- declaration -->
                        @include('backend.admission.update_partial._declaration')
                    </div>
                </form>
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        window.getCoursesByEducationUrl = "{{ route('admission.getCoursesByEducation') }}";
    </script>
    <script src="{{ asset('js/admission/admission_backend_update.js') }}"></script>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', 'input[type="file"]', function() {
                ...
                currentImageInputId = $(this).attr('id');
                readFile(this);
            });
                ...
                cropper.getCroppedCanvas().toBlob(function(blob) {
                    ...
                    reader.onloadend = function() {
                        ...
                        if (isStaticCrop) {
                            ...
                            $.post("{{ route('backend.admission.file-upload') }}", jsonObj,
                                function(
        ...
        $(document).ready(function() {
            function toggleDegreeResultsSection() {
                var selectedCourseId = $('#course_id').val() ?? $('#course_id').val();
