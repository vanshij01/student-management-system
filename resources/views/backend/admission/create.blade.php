            ...
            <div id="admission_form" class="bs-stepper mt-2">
                <form action="{{ route('admission.store') }}" method="post" enctype="multipart/form-data"
                    ...
                    <div class="bs-stepper-content p-0">
                        <!-- personal Details -->
                        @include('backend.admission.create_partial._personal_details')
                        <!-- family-details -->
                        @include('backend.admission.create_partial._family_details')
                        <!-- education-details -->
                        @include('backend.admission.create_partial._education_details')
                        <!-- declaration -->
                        @include('backend.admission.create_partial._declaration')
                    </div>
                </form>
            </div>
        </div>
...
@section('scripts')
    ...
    <script src="{{ asset('js/admission/admission_backend.js') }}"></script>
    ...
    <script>
        ...
        $(document).ready(function() {
                ...
                cropper.getCroppedCanvas().toBlob(function(blob) {
                    ...
                    reader.onloadend = function() {
                        ...
                        if (isStaticCrop) {
                            ...
                            let imgId, hiddenId;
                            imgId = inputId.replace('_doc_url',
                            'Image');
                            hiddenId = inputId.replace('_doc_url',
                            'image');
                            ...
                            $.post("{{ route('backend.admission.file-upload') }}", jsonObj, function(
            ...
            if (studentId) {
                ...
                // Show loading
                $('#student_data_loading').removeClass('d-none');

                // Fetch student details via AJAX
                $.ajax({
                    type: "GET",
                    url: "/admission/fetch-student-data/" + studentId,
                    cache: false,
                    success: function(response) {
                        $('#student_data_loading').addClass('d-none');
                        ...
                        if (response.success) {
                            ...
                            setTimeout(() => {
                                $('#course_id').append(
                                    $('<option>', {
                                        value: window.studentData.course_id,
                                        text: window.studentData.course_name,
                                        selected: true
                                    })
                                ).trigger('change');
                            }, 500);
            ...
            // 2. On selecting student from dropdown
            $('#student_id').on('change', function() {
                const student_id = $(this).val();
                if (student_id) {
                    // Redirect with student_id to reload form with their data
                    window.location.href = "{{ route('admission.create') }}?student_id=" + student_id;
                }
            });
        });
        $(document).ready(function() {
