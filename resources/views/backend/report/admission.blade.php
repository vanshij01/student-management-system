...
@section('content')
    <div class="card mb-3 filter-card">
        <div class="card-body">
            <form id="filter_form">
                <input type="hidden" name="page_length" id="page_length" value="10">
                <div class="row">
                        ...
                        <div class="form-floating form-floating-outline">
                            <select class="form-select select2" name="year" id="year"
                                ...
                                @foreach ($yearList as $item)
                                    <option @if ($request->year == $item) selected @endif
                                        value="{{ $item }}">
                                        {{ $item }}</option>
                    ...
                    <div class="col-xl-2 col-md-4 col-12 d-flex" style="column-gap: 20px;">
            ...
            @php
                ...
                if ($chk) {
                    echo '<a href="' .
                        ...
                        '" class="btn btn-primary waves-effect waves-light addButton"><i
                    class="mdi mdi-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Add Admission</span></a>';
                }
            @endphp
            {{-- <a href="{{ route('admission.create') }}" class="btn btn-primary waves-effect waves-light addButton"><i
                    class="mdi mdi-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Add Admission</span></a> --}}
        </div>
        @if (session('message'))
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        ...
                                        /* if ($updateCheck) {
                                            $html .= '<li><a class="dropdown-item dropdown-trigger-17500btn waves-effect" href="pledge-donation/' . $item->id . '/edit">Edit</a></li>';
                                        } */
                                        if ($isSuperAdmin) {
                                            $html .=
                                            ...
                                            if ($item->is_bed_release == 0) {
                                                if ($item->is_fees_paid == 0 && $item->is_admission_confirm == 0) {
                                                    $html .= '<p class="m-0 px-3 text-primary">Admission not Confirm</p>';
                                                }
                                                else if ($item->is_admission_confirm == 0) {
                                                    ...
                                                    if ($item->is_admission_confirm == 1) {
                                                        ...
                                                        $html .= '<p class="m-0 px-3 text-info"">Admission Cancelled</p>';
                                                    } else {
                                                        $html .= '<p class="m-0 px-3 text-danger">Admission Rejected</p>';
                                                    }
                                                }
                                            } else {
                                ...
                                {{-- <td>{{ $item->is_admission_confirm }}</td> --}}
                                {{-- <td>
                                    @if ($item->is_bed_release == 0)
                                        @if ($item->is_fees_paid == 0 && $item->is_admission_confirm == 0)
                                            Admission not Confirm
                                        @else
                                            @if ($item->is_admission_confirm == 1)
                                                <a href="javascript:void(0)"
                                                    class="btn btn-primary waves-effect waves-light"
                                                    onclick="payFees({{ $item->id }})">Pay</a>
                                            @elseif ($item->is_admission_confirm == 3)
                                                Admission Cancelled
                                            @else
                                                Admission Rejected
                                ...
                                {{-- <td>{{ $item->fees_status . $item->payment_method }}</td> --}}
                ...
                <div class="modal-body pb-0">
                    <div class="row">
                        <div class="table-responsive text-nowrap">
                            <table class="table" id="viewFeesReceiptTable"
    ...
    <div class="modal fade roomAllocateModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id="roomAllocateForm" action="{{ route('reservation.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="student_id" class="student_id">
                    <input type="hidden" name="admission_id" class="admission_id">
                    <div class="modal-body pb-0">
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-lg-6 mb-4">
                                <div class="form-floating form-floating-outline">
                                    <select name="hostel_id" id="hostel_id" class="select2 form-select"
                                        data-placeholder="Select Hostel" data-parsley-errors-container="#hostel_id_errors" required>
                            ...
                            <div class="col-sm-12 col-lg-6 mb-4">
                                <div class="form-floating form-floating-outline">
                                    <select name="bed_id" id="bed_id" class="select2 form-select" data-placeholder="Select Bed"
                                        data-parsley-errors-container="#bed_id_errors" required>
                        ...
                        <div class="row">
                            <div class="col-md-12 px-4">
                                ...
                                <div id="room_allotment_section" class="d-none">
                                    <div class="d-flex gap-2"><b>Hostel Name:- </b><p id="hostel_name_text"></p></div>
                                    <div class="d-flex gap-2"><b>Room Number:- </b><p id="room_name_text"></p></div>
                                    <div class="d-flex gap-2"><b>Bed Number:- </b><p id="bed_name_text"></p></div>
                                </div>
                            </div>
                        </div>
                    </div>
