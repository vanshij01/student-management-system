{{-- @props(['label', 'admissionDocuments', 'admissionDetail', 'admissionDetail'])

@php
    $percentageField = 'sem' . $label . '_percentage';
@endphp

<div class="col semester-group" data-sem="{{ $label }}">
    <label class="form-label" for="semester_{{ $label }}_backlog_backlog_result_upload">Upload your Semester-{{ $label }}
    ...
    <div class="row g-2 justify-content-between">
            ...
            <div class="d-flex">
                ...
                <div class="percentage-error">
                    @if ($admissionDetail && $admissionDocuments)
                        @php
                            $document = $admissionDocuments->firstWhere('doc_type', 'Semester ' . $label);
                        @endphp
                        @if (!empty($document) && $document->doc_type == 'Semester ' . $label && $document->course_id == $admissionDetail->course_id)
                            <input type="text" style="width: 100px" class="form-control"
                                ...
                                name="semester_{{ $label }}_backlog_percentage" value="{{ $document->percentile }}">
                        @else
                            <input type="text" style="width: 100px" class="form-control"
                                id="semester_{{ $label }}_backlog_percentage"
        ...
        <div class="col-12 col-md-8 upload_document">
            <div class="upload-group drop-area">
                <label for="semester_{{ $label }}_backlog_backlog_result_upload" class="custom-file-upload">
                    <span id="semester_{{ $label }}-backlog_backlog_result-label">SEM-{{ $label }} Backlog Result <i
                            class="las la-plus-circle" data-default="SEM-{{ $label }} Backlog Result"></i></span>

                    <input type="file" name="semester_{{ $label }}_backlog_backlog_result"
                        id="semester_{{ $label }}_backlog_backlog_result_upload"
                ...
                <input id="semester_{{ $label }}_backlog_backlog_resultimage" type="hidden" />
                <div class="mb-3 d-none" id="semester_{{ $label }}_backlog_upload_wrapper"
                ...
                <img id="semester_{{ $label }}_backlog_backlog_resultImage" class="rounded img-fluid" src=""
                    style="display: none;" />
                @if ($admissionDetail && $admissionDocuments)
                    @foreach ($admissionDocuments as $document)
                        @if ($document->doc_type == 'Semester {{ $label }} (Backlog)')
